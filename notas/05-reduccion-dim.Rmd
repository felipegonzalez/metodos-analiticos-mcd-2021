...
```{r}
tibble(pelicula=peliculas$pelicula,  afinidad= 100*personas$u[15]*peliculas$v)
```
...
# puedes bajar los datos y reproducir desde datos originales 
# ve la instrucciones en la carpeta datos/netflix del repo
if(FALSE){
  evals <- read_csv('../datos/netflix/dat_muestra_nflix.csv')
  evals
}
```
...
if(FALSE){
  ...
  saveRDS(svd_parcial, file ='salidas/svd_netflix.rds')
}
```


```{r}
svd_parcial <- readRDS('salidas/svd_netflix.rds')
svd_parcial$d
```
...
saveRDS(V_peliculas, file = 'salidas/v_peliculas.rds')
saveRDS(U_usuarios, file = 'salidas/u_usuarios.rds')
}
```

```{r}
V_peliculas <- readRDS('salidas/v_peliculas.rds')
U_usuarios <- readRDS('salidas/u_usuarios.rds')
```
...
ggplot(muestra, aes(x=v_3, y=v_4, label=etiqueta)) + geom_point(alpha=0.2) + 
  geom_text_repel(size=2.0, max.overlaps = 20)  + xlab('Dim 3') + ylab('Dim 4')
```
...
if(FALSE){
  ...
  saveRDS(evals_num_u, 'salidas/evals_num_u.rds')
}
```

```{r}
evals_num_u <- readRDS('salidas/evals_num_u.rds')
...
```{r}
evals_num_p <- readRDS('salidas/evals_num_p.rds')
