...
puede requiere menos de unos 4Gb de memoria (8Gb no son suficientes para encontrar 
...
```{r, echo=FALSE}
combs <- crossing(p = c(0, 1), c = c(0, 1), ch = c(0, 1))
probs <- combs
# fijar algunos números  
x <-  c(0, 9, 16, 16, 16, 16, 9, 4)
...
simular_transacciones <- function(n_items, n_trans, prob){
  ...
  trans <- lapply(1:n_trans, function(i){
...
obs_tbl <- tibble(long = names(conteos_obs), n = conteos_obs)
ggplot(reps_tbl, aes(x = long, y = n)) + geom_jitter() +
  geom_point(data = obs_tbl, colour = 'red', size = 3) + 
  labs(x = 'Tamaño')
```
...
tenemos un conjunto de $n$ transacciones, y buscamos soporte mínimo de $s$.


- Por ejemplo, tomar $s$ de forma que $ns=2$ no es buena idea, pues basta observar 2 transacciones de un conjunto para que el conjunto sea frecuente. Esto fácilmente puede pasar con conjuntos cuyo soporte teórico es considerablemente más bajo que $s$.


Una manera simple de obtener una guía para escoger el mínimo $s$ se puede
obtener con el siguiente argumento: para un $I$ fijo con soporte $s_I$, si 
$n$ es suficientemente grande entonces podemos usar la aproximación normal
con $E(n(I)) = ns_I$ y $var(n(I)) = ns_I(1-s_I)$. Si suponemos que nos interesa
excluir con probabilidad mayor a 0.15 a los conjuntos $I$ tales que $s_I\leq 0.75 s,$
entonces podemos demostrar que basta que 
$$s\geq \frac{1}{2\sqrt{n}}$$
**que podemos usar como guía aproximada para escoger el mínimo soporte para  un análisis dado**. Abajo mostramos una tabla de valores mínimos **sugeridos** para cada $n$:

```{r}
tibble(n = c(1e3, 1e4, 1e5, 1e6, 1e7)) %>% 
  mutate(s_min = 0.5 / sqrt(n))
```
...
```{r}
set.seed(125)
n_trans <- c(10, 25, 50, 100, 200, 400, 800, 1600, 3200, 6400, 12800)
...
```{r}
ggplot(reps_long_2, aes(x=factor(n_trans), y = n)) + geom_boxplot() +
  ylab("Número de conjuntos frecuentes (k=2)") +
  xlab("Número de transacciones") + labs(subtitle = "Soporte min 0.005")
```
...
reglas_f <- subset(b_reglas_lift, size(b_reglas_lift) == 2)
length(reglas_f)
write_csv(reglas_f %>% DATAFRAME %>% 
          rename(source = LHS, target = RHS) %>% 
          select(-count), 
          file = './salidas/reglas.csv')
```
