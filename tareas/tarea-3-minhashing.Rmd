---
title: "Similitud de documentos y minhashing"
...
# aplicamos la permutacion a 1,2,... n:
...
# calculamos firmas tomando el mínimo
...
Considera ahora la siguiente generadora de funciones, tomando un número primo grande

```{r}
generar_hash_mult <- function(p){
  a <- sample(p - 1, 1)
  b <- sample(p - 1, 1)
  # multiplicamos el valor de la permutación por un valores fijos escogidos al azar:
  hash_fun <- function(x) (a * x + b) %% p
}
```

Usamos $p = 524287$, pero para aplicaciones típicas se utilizan primos más grandes.
Por ejemplo, los valores de los renglones ahora van a dar a los siguientes números.

```{r}
set.seed(12881)
perm_mult_1 <- generar_hash_mult(524287)
perm_mult_1(1:nrow(mat)) 
```

Nótese que estos nos son números de renglones.
