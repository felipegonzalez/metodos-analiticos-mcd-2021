...
El vector de scores de renglones correspondientes indica mayor gasto proporcional
en algunas categorías sobre otras:

```{r}
tibble(rubros_nom, u = decomp_deciles$u[,1]) %>%
  arrange(desc(u)) %>% 
  knitr::kable()
```
La calidad de la aproximación de orden 1 es:

```{r}
decomp_deciles$d[1]^2 / sum(X^2)
```

De modo que la primera dimensión producee una aproximación de calidad alta.
Podemos graficar:
...
```{r}
tibble(deciles_nom, v = decomp_deciles$v[,2]) %>% knitr::kable()
```

Como el decil 9 y 10 tiene valores positivos grandes, cuando multiplicamos por
el vector 2 de scores, incrementa el gasto alimentos diversos, leche y derivados, fruta, etc.
y disminuye el gasto en cereales y verduras y legumbres. Lo opuesto sucede para
los deciles bajos como el 1 y el 2. **Esto indica que conforme a la primera aproximación
de proporcionaledad, hay una desviación que esta segunda dimensión corrige**.
...
A <- round(decomp_deciles$d[2]*tcrossprod(decomp_deciles$u[,2], decomp_deciles$v[,2])/1e3)
rownames(A) <- deciles$X1
A
```

Veamos qué tan buena es la aprximación de orden 2:

```{r}
X_2 <- decomp_deciles$d[1] * decomp_deciles$u[,1] %*% t(decomp_deciles$v[,1]) +
  decomp_deciles$d[2] * decomp_deciles$u[,2] %*% t(decomp_deciles$v[,2])
plot(X, X_2)
```
De modo que con dos dimensiones explicamos adecuadamente los patrones importantes
de  esta tabla. La dimensión 1 contribuye considerablemente más pues:

```{r}
decomp_deciles$d[1]^2 / sum(X^2)
decomp_deciles$d[2]^2 / sum(X^2)
```
